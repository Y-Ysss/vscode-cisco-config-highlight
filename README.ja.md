...
> 変更内容 (v0.5 → v0.6)  
>
> 一部のキーワードカテゴリに割り当てられているセマンティックトークンスコープが細分化されました。  
> カスタムシンタックスカラーを定義していた場合、この変更によりバージョン0.5以前のスコープ定義に依存するツールやテーマとの間に互換性の問題が発生する可能性があります。  
>
> 特定のスコープトークンで表示がうまくいかない場合、変更履歴を参照し、トークンに応じて設定を更新してください。  
